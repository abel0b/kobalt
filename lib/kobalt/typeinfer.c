...
static int typeinfer_rec(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid);

static void typeinfer_fun(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    ...
    struct kbsymbol* funsym = kbmodgraph_try_resolve(modgraph, modid, node->parent, &name_str);

    if (funsym == NULL || (funsym->kind == FunSym && !funsym->data.fun.defined)) {
        if (funsym == NULL) {
            funsym = kbmodgraph_define(modgraph, modid, node->parent, &name_str);
            funsym->kind = FunSym;
            funsym->type = (struct kbtype *) kbobjpool_alloc(&mod->astinfo.type_pool);
        }
...
static void typeinfer_group(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbnode* node = &ast->nodes.data[nid];
...
static void typeinfer_funparam(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbnode* node = &ast->nodes.data[nid];
    ...
    kbstr_new(&name_str);
    kbstr_cat(&name_str, name);
    struct kbsymbol* symbol = kbmodgraph_define(modgraph, modid, nid, &name_str);
    kbstr_del(&name_str);
...
static void typeinfer_type(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbnode* node = &ast->nodes.data[nid];
    struct kbmod* mod = kbmodgraph_get(modgraph, modid);
    struct kbtype* type = (struct kbtype *) kbobjpool_alloc(&mod->astinfo.type_pool);
...
static void typeinfer_seq(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid)  {
    struct kbnode* node = &ast->nodes.data[nid];
    struct kbmod* mod = kbmodgraph_get(modgraph, modid);
...
static void typeinfer_callparams(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbnode* node = &ast->nodes.data[nid];
    for(int i = 0; i < node->data.group.numitems; ++ i) {
        typeinfer_rec(ast, modgraph, modid, ctx, scope, node->data.group.items[i]);
    }
...
static void typeinfer_callparam(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbnode* node = &ast->nodes.data[nid];
    typeinfer_rec(ast, modgraph, modid, ctx, scope, node->data.callparam.expr);
}

static void typeinfer_strlit(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbmod* mod = kbmodgraph_get(modgraph, modid);
    struct kbtype* str_type = (struct kbtype *) kbobjpool_alloc(&mod->astinfo.type_pool);
    kbtype_new(str_type, Str);
    mod->astinfo.types.data[nid] = str_type;
    kbvec_type_push(&ctx->types, str_type);
}

static void typeinfer_floatlit(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbmod* mod = kbmodgraph_get(modgraph, modid);
    struct kbtype* float_type = (struct kbtype *) kbobjpool_alloc(&mod->astinfo.type_pool);
    kbtype_new(float_type, Float);
    mod->astinfo.types.data[nid] = float_type;
    kbvec_type_push(&ctx->types, float_type);
}

static void typeinfer_intlit(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbmod* mod = kbmodgraph_get(modgraph, modid);
    struct kbtype* int_type = (struct kbtype *) kbobjpool_alloc(&mod->astinfo.type_pool);
    kbtype_new(int_type, Int);
    mod->astinfo.types.data[nid] = int_type;
    kbvec_type_push(&ctx->types, int_type);
}

static void typeinfer_charlit(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbmod* mod = kbmodgraph_get(modgraph, modid);
    struct kbtype* char_type = (struct kbtype *) kbobjpool_alloc(&mod->astinfo.type_pool);
    kbtype_new(char_type, Char);
    mod->astinfo.types.data[nid] = char_type;
    kbvec_type_push(&ctx->types, char_type);
}

static void typeinfer_id(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbmod* mod = kbmodgraph_get(modgraph, modid);
    struct kbnode* node = &ast->nodes.data[nid];
    struct kbstr name_str;
    kbstr_new(&name_str);
    kbstr_cat(&name_str, node->data.id.name);
    mod->astinfo.types.data[nid] = kbmodgraph_resolve(modgraph, modid, nid, &name_str)->type;
    kbstr_del(&name_str);

    kbvec_type_push(&ctx->types, mod->astinfo.types.data[nid]);
}

static void typeinfer_call(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbnode* node = &ast->nodes.data[nid];
    struct kbmod* mod = kbmodgraph_get(modgraph, modid);
...
static void typeinfer_ifelse(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbnode* node = &ast->nodes.data[nid];
    struct kbmod* mod = kbmodgraph_get(modgraph, modid);
    for(int i = 0; i < node->data.group.numitems - 1; ++i) {
        typeinfer_rec(ast, modgraph, modid, ctx, scope, node->data.group.items[i]);
...
static void typeinfer_ifbranch(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbnode* node = &ast->nodes.data[nid];
...
static void typeinfer_elsebranch(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* scope, int nid) {
    struct kbnode* node = &ast->nodes.data[nid];
	typeinfer_rec(ast, modgraph, modid, ctx, scope, node->data.ifbranch.conseq);
}

static int typeinfer_rec(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid, struct kbtypeinfer_ctx* ctx, struct kbscope* pscope, int nid) {
    struct kbnode* node = &ast->nodes.data[nid];
    struct kbmod* mod = kbmodgraph_get(modgraph, modid);
    ...
    if (has_scope(node)) {
        scope = kbastinfo_alloc_scope(&mod->astinfo, pscope);
    }
...
void kbtypeinfer(struct kbast* ast, struct kbmodgraph* modgraph, struct kbstr* modid) {
    ...
    // struct kbtype* type = (struct kbtype*)kbmodgraph_resolve(astinfo, "writeln", 0);
