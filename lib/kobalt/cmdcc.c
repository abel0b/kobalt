...
void kbcmdcc_compile(struct kbopts* opts, struct kbcmdcc* cmdcc, struct kbstr* src, struct kbstr* bin) {
    ...
    kbstr_catf(&logfilepath, "%s/cc.log", opts->cachedir);
    ...
    char* args[] = {ccoptextra[cmdcc->cc], binopt, src->data, NULL};
#else
    char* args[] = {ccoptextra[cmdcc->cc], ccoptoutput[cmdcc->cc], bin->data, src->data, NULL};
#endif
