...
static void checkptr(void* ptr, size_t size) {
    if (ptr == NULL) {
        kl_elog("dynamic memory allocation of %zuB failed", size);
        exit(1);
    }
}
...
void* kl_malloc_aux(size_t size, const char* file, int line, const char* func) {
    if (getenv("DEBUG_MEMORY")) {
        kl_ilog("mem.malloc: %zuB at %s:%d:%s", size, file, line, func);
    } 
#else
...
void* kl_realloc_aux(void * ptr, size_t newsize, const char* file, int line, const char* func) {
    if (getenv("DEBUG_MEMORY")) {
        kl_ilog("mem.realloc: %p %zuB at %s:%d:%s", ptr, newsize, file, line, func);
    }
...
void kl_free_aux(void * ptr, const char* file, int line, const char* func) {
    if (getenv("DEBUG_MEMORY")) {
        kl_ilog("mem.free: %p at %s:%d:%s", ptr, file, line, func);
    }
#else
