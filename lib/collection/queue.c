...
void kbqueue_enqueue(struct kbqueue* queue, void* elem) {
    if (queue->size == queue->capacity && queue->size) {
        ...
        if (queue->front + queue->size > prev_cap) {
            memcpy(&queue->data[(prev_cap - 1) * queue->elem_size], &queue->data[0], queue->elem_size);
        }
    }
    ++ queue->size;
    memcpy(&queue->data[queue->elem_size * ((queue->front + queue->size) % queue->capacity)], elem, queue->elem_size);
}

void kbqueue_dequeue(struct kbqueue* queue, void* elem) {
    queue->size --;
    memcpy(elem, &queue->data[queue->elem_size * queue->front], queue->elem_size);
